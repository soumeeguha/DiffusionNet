...
image_features = []
img_num = 1
images = root + 'experimental_imgs/test_imgs/*.png'
for filename in get_image_list(images):
    print(filename)
    features, image_cropped = get_image_features(filename, kernels)
    if (features is not None) and (image_cropped is not None):
        cv2.imwrite(root + f'experimental_imgs/cropped_test_imgs/img{img_num}.png', image_cropped*255)
        print(root + f'experimental_imgs/cropped_test_imgs/img{img_num}.png')
        image_features.append(features)
        img_num += 1
        print(img_num)
df = pd.DataFrame(image_features)
df.to_csv(root + f'renamed_imgs/features/features_test.csv', index= False)
